...
const ShowInvoices = () => {
  ...
  const ExportCSV = () => {
    ...
    const ItemData = processedInvoices.map((invoice) => {
    ...
    const data = processedInvoices.map((invoice, index) => ({
      ...
      itemsName: ItemData[index].map((item) => item.itemName).join(", "),
      itemsFee: ItemData[index].map((item) => item.fees).join(", "),
      itemsDiscount : ItemData[index].map((item) => item.discount).join(", "),
      ItemsAmount : ItemData[index].map((item) => item.amount).join(", "),
  ...
  return (
    <div className="md:mx-[10vw] mx-[20px]">
      <h1 className="text-lg mt-2 font-semibold text-gray-900 dark:text-gray-400">
        Invoices
      </h1>
      ...
      {invoiceLoading || studentsLoading || (itemsLoading && <LoadingModel />)}
